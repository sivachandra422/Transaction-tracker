...
export default function AndroidFrame({ children, title, onBack, showBack = false, actions, theme = "light", bottomNav }: AndroidFrameProps) {
  ...
  return (
    <div className={`min-h-screen bg-white dark:bg-[#0b121f] sm:bg-slate-900 flex items-center justify-center p-0 sm:p-6 font-sans transition-colors duration-200 ${theme}`}>
      ...
      <div className="w-full sm:w-[412px] sm:h-[860px] sm:rounded-[40px] sm:border-[8px] sm:border-slate-800 bg-slate-50 dark:bg-[#0b121f] flex flex-col relative overflow-hidden sm:shadow-2xl transition-colors duration-200">
        ...
        {/* Top Android Status Bar — only visible inside the desktop phone frame */}
        <div className="hidden sm:flex bg-[#1e293b] dark:bg-[#080d19] text-slate-200 dark:text-slate-400 px-6 py-2 pb-1.5 justify-between items-center text-xs font-semibold select-none z-40 relative border-b border-white/5 transition-colors">
          <span>{time}</span>
        ...
        {/* Bottom Material Touch navigation pill — only visible inside the desktop phone frame */}
        <div className="hidden sm:flex bg-slate-950 dark:bg-[#040711] text-slate-400 py-3 px-12 justify-between items-center select-none border-t border-slate-900 dark:border-white/5 z-40 transition-colors">
